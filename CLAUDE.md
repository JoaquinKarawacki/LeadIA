...
- **Import de catálogo por PDF** (`src/utilidades/flujos/importar-catalogo-pdf.ts`): el candidato más pesado de la etapa (catálogos reales de cientos de páginas), así que corre en **Vercel Workflow DevKit** (paquete `workflow`, `next.config.ts` envuelto con `withWorkflow()`) en vez de bloquear la request o encadenar `fetch`+`after()` a mano — decisión tomada explícitamente con el usuario por los reintentos/resumibilidad de fábrica que da WDK. Flujo: `subirCatalogoPdf` (`src/app/catalogo/importar/acciones.ts`) extrae el texto del PDF **sincrónicamente** con `pdf-parse` (rápido, no hace falta Storage — el array de texto por página se pasa directo como argumento del workflow) y crea la fila en `importacion_catalogo`; el workflow trocea las páginas en lotes de 6 (`PAGINAS_POR_LOTE`), cada lote es un step (`estructurarLote`) que llama a `completarEstructurado()` (nuevo en `llm.ts`, `generateObject` con schema de `zod`) para extraer productos, y otro step (`guardarLote`) persiste el resultado. Pantalla de revisión editable en `/catalogo/importar/[id]` antes de guardar en `producto` (sin componentes cliente, mismo patrón de forms que el resto).
- **Gotchas reales encontrados probando con un catálogo real de 244 páginas** (ver memoria `decisiones_import_pdf`): (1) Turbopack rompe la resolución del worker interno de `pdfjs-dist`/`pdf-parse` ("Setting up fake worker failed") — se resuelve con `serverExternalPackages: ["pdf-parse", "pdfjs-dist"]` en `next.config.ts`. (2) El free tier del AI Gateway tiene rate limit real, que corta un import largo a mitad de camino — `estructurarLote` detecta `GatewayRateLimitError` (incluso envuelto en `AI_RetryError`, hay que mirar `error.lastError`) y tira `RetryableError` con `retryAfter: "90s"` en vez de dejar que el AI SDK reintente rápido y se rinda (por eso `completarEstructurado` pasa `maxRetries: 0` al SDK cuando corre en background — el backoff real lo maneja el step). (3) Muchos catálogos B2B **no imprimen precio** (se cotiza aparte) — el schema de extracción lo deja `nullable`, nunca inventado, y la pantalla de revisión fuerza a completarlo a mano antes de guardar.

...
- Con esto se cumple el DoD de la Etapa 2 ("metés una empresa real y devuelve productos relevantes con explicación"). Falta para cerrar la etapa: import de catálogo por PDF (extracción de texto + modelo chico que estructura filas + pantalla de revisión).

Hay dos tenants de prueba cargados en la base (`demo@leadia.test` / `demo2@leadia.test`, contraseñas en el historial de este chat) — son solo para verificar aislamiento, se pueden borrar cuando se sume el primer cliente real. También se sumaron y ya se borraron los tenants "Prueba Etapa2 Investigacion" y "Prueba Etapa2 Matching": la extensión de Chrome sigue sin conectar en este entorno (ver memoria `entorno_sin_extension_chrome`), así que ninguna de las dos features (investigación, matching) se validó clickeando en el navegador — se verificaron con scripts descartables que replican el camino real (login real, llamadas a Exa/Gateway, inserts respetando RLS, y en el caso de matching, insertando dos productos con embeddings distintos y confirmando que el RPC `buscar_productos_similares` ordena por similitud real, no al azar).
